...
5. **Update Documentation** _(required before committing)_:
   - **Simple fix**: Add an entry to `docs/fixes/{{YYYY-MM}}.md` (create the file if it doesn't exist for the current month)
   - **Complex fix** (multi-file, architectural, data migration): Create `docs/fixes/{{ISSUE_ID}}-FIX-NAME/` with `spec.md` (root cause) and `plan.md` (fix approach)
   - **Breaking change**: Update `CHANGELOG.md`
   - Do **not** proceed to commit until the docs entry is written
